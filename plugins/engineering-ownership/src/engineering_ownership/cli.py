...
def index_rows(root: Path, contract: dict[str, Any]) -> list[dict[str, Any]]:
    """One row per change record, plus any decision document no record claims.

    An ADR written at R1 has nowhere to live in the record's artifact map,
    because artifact_paths allocates a decision document only from R2 up. Such a
    file is reachable by `ls` and by nothing else — not `explain`, not
    `refs check`. Listing it here is the difference between an index and a
    prettier `status`.
    """
    rows: list[dict[str, Any]] = []
    claimed: set[str] = set()
    for record in list_evidence(root, contract):
        artifacts = record.get("artifacts", {})
        claimed.update(artifacts.values())
        closed = record.get("closed")
        rows.append(
            {
                "id": record["change_id"],
                ...
                "risk": record["risk"],
                "state": (
                    f"closed {str(closed.get('closed_at', ''))[:10]}" if closed else "open"
                ),
                "revision": str(closed.get("revision", ""))[:12] if closed else "",
                "artifacts": sorted(artifacts.items()),
                "orphan": "",
            }
        )

    decisions_dir = contract.get("artifacts", {}).get("decisions")
    if decisions_dir:
        directory = safe_relative_path(root, decisions_dir, for_write=True)
        if directory.is_dir():
            for path in sorted(directory.glob("*.md")):
                relative_path = relative(root, path)
                if relative_path in claimed or path.name == "README.md":
                    continue
                rows.append(
                    {
                        "id": path.stem,
                        ...
                        "state": "decision only",
                        "revision": "",
                        "artifacts": [("decision", relative_path)],
                        "orphan": "no evidence record claims this document",
                    }
                )
    return rows


def link_from(base: str, target: str) -> str:
    """Repository-relative target, rewritten relative to the index's own directory.

    A link is only useful where the file sits. Writing the index into
    docs/engineering/ and leaving root-relative paths in it produces a table of
    dead links, which is worse than no table.
    """
    if not base:
        return target
    ...
    return "/".join([*up, *target_parts[common:]]) or "."


SKILL_REFERENCES = "plugins/engineering-ownership/skills/engineering-ownership/references"


def index_text(
    root: Path, contract: dict[str, Any], as_markdown: bool, base: str = ""
) -> str:
    rows = index_rows(root, contract)
    if not as_markdown:
        lines = [f"{row['id']}: {row['risk']} {row['state']} - {row['title']}" for row in rows]
        return "\n".join(lines) + ("\n" if lines else "")
    ...
    for row in rows:
        state = row["state"]
        if row["revision"]:
            state += f" at `{row['revision']}`"
        if row["orphan"]:
            state += f" — {row['orphan']}"
        links = ", ".join(
            f"[{kind}]({link_from(base, path)})" for kind, path in row["artifacts"]
        )
        lines.append(
            f"| **{row['title']}**<br>`{row['id']}` | {row['risk']} | {state} | "
            f"{links or '-'} |"
        )
    operating = link_from(base, f"{SKILL_REFERENCES}/operating-model.md")
    evidence = link_from(base, f"{SKILL_REFERENCES}/evidence-model.md")
    lines.extend(
        [
            "",
            f"{len(rows)} entr{'y' if len(rows) == 1 else 'ies'}. Risk tiers R0-R3 are defined "
            f"in the skill's [operating model]({operating}); what each evidence record holds "
            f"is described in the [evidence model]({evidence}).",
            "",
        ]
    )
    return "\n".join(lines)
...
def command_index(args: argparse.Namespace) -> int:
    root = repo_root(args.repo)
    contract = read_contract(root)
    base = PurePosixPath(args.write).parent.as_posix() if args.write else ""
    output = index_text(
        root, contract, as_markdown=args.format == "md", base="" if base == "." else base
    )
    if args.write:
        path = write_repo_text(root, args.write, output, overwrite=True)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    status.add_argument(
        "--due", action="store_true", help="Only records whose revisit date has passed"
    )
    status.add_argument(
        "--all", action="store_true", help="Include closed records, not just open work"
    )
    status.set_defaults(func=command_status)

    index = sub.add_parser("index", help="List every change record with its title and documents")
    index.add_argument("--format", choices=("md", "text"), default="text")
    index.add_argument("--write", help="Repository-relative path to write the index to")
