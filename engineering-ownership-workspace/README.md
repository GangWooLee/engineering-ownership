...
| `iteration-6` | Trigger probes | Whether the skill is consulted at all, measured against a probe set with both a positive and a negative class, and keyed by which description each run was taken against. **Not an evaluation result.** The probes were committed before the description was rewritten, so the ordering is checkable in history. |
