...
| `iteration-8-eval9-regrade` | Re-grade, no new runs | The six stored `iteration-8` eval-9 responses judged again under a corrected rubric: the expectation that had never passed in any run is removed, and the two that require a durable artifact admit a deferral branch, as all six original judges independently recommended. Responses are byte-identical to `iteration-8`, which is untouched. Tests whether that scenario's negative gap was a rubric artifact. |
| `iteration-8` | Efficacy, complete | The rebuilt efficacy attempt, against the plugin as published in `v0.3.0`. Nine scenarios, three runs per configuration, preflight run rather than skipped because the skill changed since the abandoned attempt. Collected, graded, and corrected with nothing excluded. **No number from it is published**, and its own note carries a correction overturning two of its first readings — read that before any figure. |
| `iteration-7` | Efficacy, abandoned | The first attempt to ask whether the skill helps rather than whether it runs. **Abandoned on 2026-07-26 and deliberately left uncommitted.** The sweep was cut off during eval-8, and the plugin changed materially the next morning, so adding the missing runs would have put two skill versions in one arm — the confound the withdrawn evaluation was retracted over. Its own note records what was collected and why it was discarded. A replacement is collected against a settled plugin. |
