...
| `iteration-8` | Efficacy, collecting | The rebuilt efficacy attempt, against the plugin as published in `v0.3.0`. Nine scenarios, three runs per configuration, preflight run rather than skipped because the skill changed since the abandoned attempt. **Collecting — read nothing from it until its own note says the sweep finished and reports what was excluded.** |
